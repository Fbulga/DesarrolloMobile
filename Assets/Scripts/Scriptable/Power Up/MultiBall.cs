...
public class MultiBall : EffectSO
{
    ...
    {
        var activeBalls = PoolManager.Instance.ActivePool[ArkanoidManager.Instance.ballPrefab];
        ...
        foreach (GameObject ball in ballsCopy)
        {
            ...
            {
                if (PoolManager.Instance.ActivePool[ArkanoidManager.Instance.ballPrefab].Count <=
                    ArkanoidManager.Instance.ActiveBallsLimit)
                {
                    var newBall = PoolManager.Instance.GetBall(ArkanoidManager.Instance.ballPrefab,ball.transform.position);
